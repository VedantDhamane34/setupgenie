    ...
    <div className="ovl" onClick={onClose}>
      <div className="pcmdl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="pctabs">
          <button
            className={`pctab${tab === "compare" ? " on" : ""}`}
            onClick={() => setTab("compare")}
          >
            🏪 Compare Prices
          </button>
          <button
            className={`pctab${tab === "alternatives" ? " on" : ""}`}
          ...
          <div className="pccontent">
            {state === "loading" && (
              <div className="pcloading">
                ...
                <div className="pcloadtxt">
                  Checking prices across platforms…
                </div>
                <div className="pcloadtxt2">
                  Amazon · Flipkart · Croma · Reliance · Tata Cliq
                </div>
              </div>
            )}
            {state === "error" && (
              <div className="pcerr">
                ...
                <button className="pcretry" onClick={fetchPrices}>
                  Retry →
                </button>
              </div>
            )}
            {state === "done" && (
              <>
                <div className="pcestimate-banner">
                  ⚠️ These are AI-estimated prices for reference only. Always
                  check the platform for actual price before buying.
                </div>
                {savings > 0 && cheapest && (
                  <div className="pcsave">
                    🏆 Best price on <strong>{cheapest.name}</strong> — ₹
                    {cheapest.price.toLocaleString("en-IN")} · saves ₹
                  ...
                  {prices.map((p, i) => (
                    <div key={i} className={`pcrow${!p.inStock ? " oos" : ""}`}>
                      <div className="pcplat">
                        ...
                        <div>
                          <div className="pcpname" style={{ color: p.color }}>
                            {p.name}
                          </div>
                          {p.emi && p.inStock && (
                            <div className="pcemi">EMI available</div>
                          )}
                        </div>
                      </div>
                      <div className="pcprice">
                        {p.inStock ? (
                          <>
                            <span
                              className={`pcamt${i === 0 && p.inStock ? " cheapest" : ""}`}
                            >
                              ₹{p.price.toLocaleString("en-IN")}
                            </span>
                            {i === 0 && <span className="pcbest">BEST</span>}
                            {p.price > item.price && (
                              <span className="pcmore">
                                +₹
                                {(p.price - item.price).toLocaleString("en-IN")}
                              </span>
                            )}
                            {p.price < item.price && (
                              <span className="pcsavebdg">
                                -₹
                                {(item.price - p.price).toLocaleString("en-IN")}
                              </span>
                            )}
                          </>
                        ) : (
                ...
                <div className="pcnote">
                  ⚡ Prices are AI-estimated. Phase 2 will show live prices.
                </div>
              </>
            )}
          </div>
                      ...
                      <div key={i} className="altcard">
                        <div className="altcard-top">
                          ...
                          <div className="altcard-right">
                            <div
                              className={`altcard-price ${diff < 0 ? "chp" : "exp"}`}
                            >
                              ₹{Number(a.price).toLocaleString("en-IN")}
                            </div>
                            <div
                              className={`altcard-diff ${diff < 0 ? "chp" : "exp"}`}
                            >
                              {diff < 0
                                ? `Save ₹${Math.abs(diff).toLocaleString("en-IN")}`
                        ...
                        <div className="altcard-links">
                          <a
                            className="altbuy az"
                            href={`https://www.amazon.in/s?k=${encodeURIComponent(a.name)}&tag=setupgenie-21`}
                            target="_blank"
                            rel="noopener noreferrer"
                          >
                            🛒 Amazon
                          ...
                          <a
                            className="altbuy fk"
                            href={`https://www.flipkart.com/search?q=${encodeURIComponent(a.name)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                          >
                            🛍️ Flipkart
                          ...
                          <a
                            className="altbuy cr"
                            href={`https://www.croma.com/search?q=${encodeURIComponent(a.name)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                          >
                            🔴 Croma
