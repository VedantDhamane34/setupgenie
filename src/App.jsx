...
export default function App() {
  ...
  function goLanding() {
    ...
    setQi(0); setAns({}); setSetup(null);
    setItems([]); setCarted(false); setCopied(false);
    setGenErr(""); setShowConfetti(false);
  }
  ...
  async function startGenerate(answers) {
    ...
    try {
      ...
      const newSave = {
        id: Date.now(), setup:result,
        answers, date:new Date().toLocaleDateString("en-IN")
      };
      ...
      const msg = e.message?.includes("429")?"Too many requests — wait 30s and retry"
        :e.message?.includes("401")?"API key issue — please check .env"
        :e.message?.includes("Parse")?"AI response unclear — please retry"
        :e.message?.includes("No items")?"No items returned — try higher budget"
  ...
  return (
    <>
      ...
      {compareTarget && (
        <PriceCompareModal item={compareTarget} onClose={()=>setCompareTarget(null)}/>
      )}
      {showSaved && (
        <SavedModal saves={saves} onLoad={handleLoadSave}
          onDelete={handleDeleteSave} onClose={()=>setShowSaved(false)}/>
      )}

      {/* Layout */}
      <div className="G"/><div className="GL"/>
      <Topbar auth={auth} page={page} setPage={setPage}
        saves={saves} setShowSaved={setShowSaved}/>
      ...
      {page===PAGES.AUTH && (
        <Auth auth={auth} setPage={setPage}/>
      )}
      {page===PAGES.QUIZ && (
        <Quiz qi={qi} setQi={setQi} ans={ans} setAns={setAns}
          genErr={genErr} setGenErr={setGenErr}
          canGo={canGo} goNext={goNext} goBack={goBack}
          startGenerate={startGenerate} setPage={setPage}/>
      )}
      {page===PAGES.GEN && (
        <Generating genMsg={genMsg}/>
      )}
      {page===PAGES.RESULTS && setup && (
        <Results
          setup={setup} items={items} setItems={setItems} ans={ans}
          saves={saves} setSaves={setSaves}
          setSwapTarget={setSwapTarget} setCompareTarget={setCompareTarget}
          setPage={setPage} setQi={setQi} setAns={setAns}
          setSetup={setSetup} setCarted={setCarted} carted={carted}
          copied={copied} setCopied={setCopied}
          startGenerate={startGenerate} goLanding={goLanding}/>
      )}
    </>
  );
}